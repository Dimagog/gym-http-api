...
class Client
  getter remote_base
  getter session : HTTP::Client
  @remote_base : String

  def initialize(@remote_base)
    pp! @remote_base
    @session = HTTP::Client.new(URI.parse(@remote_base))
    pp! @session.port
  end
  ...
  def post_request(route, data)
    ...
    Log.logger.info "POST #{url}\n#{data}"
    resp = session.post url, headers: HTTP::Headers{"Content-Type" => "application/json"}, body: data.to_json # , content_type: :json
